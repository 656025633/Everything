...
public class MyService extends Service implements myinterface,DataTypeFormat {
    ...
    @Override
    public void getVoice(ImBean bean) throws IOException {
        //
        Log.d("random","voice");
        //把语音写到本地文件
        //将语音保存
        String sendpath = null;
        byte voicecontent [] = bean.getContent();
        try {
            sendpath = Environment.getExternalStorageDirectory().getCanonicalPath().toString()
                    + "/MessageMediaReceive"+"/"+String.valueOf(System.currentTimeMillis())+".mp3";
            getFileFromBytes(voicecontent,sendpath);
            bean.setPath(sendpath);
            EventBus.getDefault().post(bean);
        } catch (IOException e) {
            e.printStackTrace();
        }
